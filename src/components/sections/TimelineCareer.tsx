...
export function TimelineCareer({ steps }: Props) {
  ...
  return (
    <div ref={containerRef} className="relative md:h-[100svh] md:overflow-hidden">
      <ol
        ref={trackRef}
      ...
      >
        {steps.map((step, i) => {
          ...
          return (
            <li
              key={step.meta.slug}
              className="md:w-[min(620px,80vw)] md:shrink-0"
            >
              <article className="rounded-2xl border border-border bg-card/50 p-6 backdrop-blur-sm transition-colors md:p-8">
                <header className="mb-4 flex flex-wrap items-baseline justify-between gap-2 border-b border-border pb-3">
                  <span
                    className={[
                      ...
                      isCurrent
                        ? "text-[oklch(0.70_0.28_240)]"
                        : "text-foreground/50",
                    ].join(" ")}
                  >
                    {String(i + 1).padStart(2, "0")} ·{" "}
                    {formatDate(step.meta.startDate)} —{" "}
                    {step.meta.endDate
                      ? formatDate(step.meta.endDate)
                      : t("present")}
                  </span>
                  {step.meta.location ? (
                ...
                <p className="text-sm text-[oklch(0.70_0.28_240)]">
                  {step.meta.company}
                </p>
                <h4
                  className="mt-3 font-mono text-sm text-foreground/55"
                  lang={locale}
                >
                  {step.title}
                </h4>

                <div className="mt-4 text-sm leading-relaxed text-foreground/75">
                  {step.body}
                </div>
